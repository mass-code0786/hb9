create table if not exists product_delivery_links (
  ...
  title text not null,
  url text not null,
  type text not null default 'ebook' check (type in ('ebook', 'video', 'course', 'folder', 'pdf')),
  category text,
  thumbnail_url text,
  sort_order integer not null default 0,
  is_active boolean not null default true,
  created_at timestamptz not null default now(),
  unique (product_id, title, url)
);

create index if not exists idx_product_delivery_links_product_active
  on product_delivery_links (product_id, is_active, sort_order);

create table if not exists product_delivery_access_logs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references hb_users(id) on delete cascade,
  package_purchase_id uuid not null references hb_package_purchases(id) on delete cascade,
  product_delivery_link_id uuid not null references product_delivery_links(id) on delete cascade,
  action text not null default 'view' check (action in ('view', 'open', 'download', 'copy')),
  created_at timestamptz not null default now()
);

create index if not exists idx_product_delivery_access_logs_user_purchase
  on product_delivery_access_logs (user_id, package_purchase_id, created_at desc);

create index if not exists idx_product_delivery_access_logs_link
  on product_delivery_access_logs (product_delivery_link_id, action);
