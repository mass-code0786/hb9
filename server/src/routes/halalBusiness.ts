...
hbRouter.post("/hb/onchain/purchases/track", requireHbUser, asyncHandler(async (req, res) => {
  ...
  const rows = await query(
    `insert into hb_onchain_purchase_events
     ...
     on conflict (chain_id, tx_hash, log_index) where log_index is not null do nothing
     returning *`,
    [
      parsed.data.txHash,
      hbOnchainChainId(),
...
hbRouter.post("/hb/products/:id/buy", requireHbUser, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (existingRows.rows[0]) {
      await client.query("commit");
      ok(res, { order: existingRows.rows[0], idempotent: true }, "Product order already processed");
      return;
    }
    ...
    await client.query("update hb_products set stock = greatest(stock - 1, 0), updated_at = now() where id = $1", [product.id]);
...
hbRouter.post("/hb/auth/wallet/verify", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await client.query("commit");
    ok(res, { token: await createToken(user.id, walletAddress, req), user }, "HB9 wallet login successful");
  } catch (err) {
    await client.query("rollback");
    throw err;
  } finally {
