      ...
      <Panel>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-400">Manage local wallet protections and device access.</p>
      ...
      <Panel>
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm leading-6 text-slate-300">
            Use a unique wallet password, verify every recipient address, and ignore unexpected token airdrops.
          </div>
        </div>
      </Panel>
    </div>
...
function SecurityHeader({ title, value }: { title: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className="rounded-full border border-white/10 bg-[#0b1728]/60 px-3 py-1 text-xs text-slate-300">{value}</span>
    </div>
  );
}
